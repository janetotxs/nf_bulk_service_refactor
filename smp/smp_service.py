...
def process_sequence_smp():
    ...
    try:
        ...
        wd.wait_until_element(
            "name", get_env_variable("SMP_LOGIN_USERNAME_NAME"), "visible"
        )
...
def login_sequence(wd):
    try:
        ...
        wd.perform_action(
            "name", get_env_variable("SMP_LOGIN_USERNAME_NAME"), "sendkeys", username
        )
        wd.perform_action(
            "name", get_env_variable("SMP_LOGIN_PASSWORD_NAME"), "sendkeys", password
        )

        ...
        try:
            ...
            for (
                ...
                if double_extend_true:
                    ...
                    step_type_data, dict_incharge_extend_data = (
                        nf_start_steps_flow_construct(
                            double_extend_value, bs_row_data, param_worksheet, row
                        )
                    )
                    # Store the IN CHARGE and EXTEND FIRST EXPIRY data to a temporary variable dictionary to use it for double flow process.
                    if (
                        double_extend_value != "double"
                        and double_extend_value != "extend"
                    ):
                        logger.info(
                            f"STORING DATA TO TEMP. VARIABLE: {dict_incharge_extend_data}"
                        )
                        in_charge_extend_data.update(dict_incharge_extend_data)

                    elif double_extend_value == "double":
                        logger.info(
                            f"RE-USING IN CHARGE AND EXTEND FIRST EXPIRY DATA: {dict_incharge_extend_data}"
                        )
                        step_type_data.update(in_charge_extend_data)
...
def nf_start_steps_flow_construct(
    double_extend_value, bs_row_data, param_worksheet, row
):
    # --------------------START STEP PROCESS------------------------#
    try:

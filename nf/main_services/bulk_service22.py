...
class NFService:
    ...
    def nf_start_bulk_services(self):
        ...
        for i, row in enumerate(pending_rows):
            try:
                ...
                self.wd.redirect_to_page(url_bs_add_page)
                ...
                try:
                    ...
                    self.wd.perform_action(
                        "name", nf.NF_BS_QUEUE_LIMIT, "sendkeys", 1000
                    )

                    # Handle Deprov on Empty Checkbox
                    self.handle_bs_deprov_on_empty(
                        row_data[nf.NF_INDEX_DEPROV_ON_EMPTY]
                    )
                    ...
                    self.handle_bs_subscription_less(
                        row_data[nf.NF_INDEX_SUBSCRIPTION_LESS]
                    )
                    ...
                    self.wd.perform_action(
                        "name", nf.NF_BS_MAX_DAILY_EXTENSION, "clear"
                    )
                    self.wd.perform_action(
                        "name",
                    ...
                    self.wd.perform_action(
                        "name", nf.NF_BS_MAX_TOTAL_EXTENSION, "clear"
                    )
                    self.wd.perform_action(
                        "name",
                    ...
                    self.wd.wait_until_element(
                        "xpath", "//div[@class='success']", "visible"
                    )
                ...
                es.create_service_expiry(self, row_data, service_id)
                ...
                try:
                    ...
                    logger.info(
                        f"\nWorksheet Updated: {self.bs_worksheet}\nRow Updated: {row}"
                    )
                except Exception as e:
                    logger.info(
    ...
    def handle_nf_bs_include_group_status_inquiry(self, nf_group_status_inquiry_value):
        try:
            element = None
    ...
    def process_wallet(self, wallet_array):
        try:
            ...
            if wallet_array[0] == True:
                ...
                self.wd.perform_action(
                    "xpath",
                    "//option[contains(text(),'" + wallet_array[1] + "')]",
                    "click",
                )
                return
            else:
                logger.info("WALLET NOT FOUND, creating wallet...")
                self.create_nf_ds_new_wallet(wallet_array[1])
    ...
    def create_nf_ds_new_wallet(self, wallet_name):
        try:
            ...
            logger.info(
                f"An error has occurred on 'create_nf_ds_new_wallet'\nERROR: {e}"
            )
    ...
    def nf_add_service_expiry(self, row_data, service_id):
        try:
